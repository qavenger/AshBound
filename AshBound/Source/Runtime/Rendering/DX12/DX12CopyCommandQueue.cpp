...
bool DX12CopyCommandQueue::Initialize(ID3D12Device* device)
{
	if (!device)
	{
		return false;
...
void DX12CopyCommandQueue::Shutdown()
{
	if (m_fenceEvent)
	{
...
ID3D12CommandQueue* DX12CopyCommandQueue::GetQueue() const
{
	return m_queue.Get();
}

void* DX12CopyCommandQueue::GetNativeQueue() const
{
	return m_queue.Get();
}

void DX12CopyCommandQueue::Execute(IRHICommandList* commandList)
{
...
void DX12CopyCommandQueue::Flush()
{
	if (!m_queue || !m_fence)
	{
		return;
